...
* schema中字段数比较多, 聚合模型中可能key列比较多, 导入过程中需要排序的列会增加，导致导入时间变长。
* 维度信息更新会反应到整张表中，而更新的频率直接影响查询的效率，宽表的更新成本。
* 宽表的建设需要额外的开发工作、存储空间和数据backfill的成本。

使用过程中，建议优先使用星型模型，可以在保证灵活的基础上获得高效的指标分析效果，但是对于有高并发或者低延迟要求的业务，还是可以选择宽表模型进行加速，StarRocks也可以提供媲美ClickHouse的宽表查询性能。
