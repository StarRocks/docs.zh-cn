...
StarGo 是一个用于管理多个 StarRocks 集群的命令行工具。通过 SR-CTL，您可以使用简单的命令行实现多集群的部署、查看、升级、启动与停止等操作。
...
如果您在配置文件中设定的部署路径不存在，且您有创建该路径的权限，SR-CTL 将根据配置文件自动创建部署目录。如果路径已存在，请确保您有在该路径下拥有写入的权限。您也可以通过以下命令，在各部署节点分别创建部署路径。
