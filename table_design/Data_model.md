...
- `enable_persistent_index`：是否持久化主键索引，同时使用磁盘和内存存储主键索引，避免主键索引占用过大内存空间。取值为 `true` 或者 `false`。如果磁盘为固态硬盘 SSD，则建议设置为 `true`。一般情况下，持久化主键索引后，主键索引所占内存为之前的1/10。
   >  - 主键必须为定长，不支持为可变长字符串VARCHAR。
