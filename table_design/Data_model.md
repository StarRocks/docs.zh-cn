...
StarRocks 在 1.19 版本推出了主键模型（Primary Key）。主键模型的表要求有唯一的主键，支持对表中的行按主键进行更新和删除操作。相较更新模型，主键模型可以更好地支持**实时和频繁更新**等场景。
...
需要注意的是：由于存储引擎会为主键建立索引，而在导入数据时会把主键索引加载在内存中，所以主键模型对内存的要求比较高，还不适合主键特别多的场景。**目前primary主键存储在内存中，为防止滥用造成内存占满，限制主键字段长度全部加起来编码后不能超过127字节**。目前比较适合如下两个情形。
...
7. `enable_persistent_index`：是否持久化主键索引，同时使用磁盘和内存存储主键索引，避免主键索引占用过大内存空间。取值为 `true` 或者 `false`。如果磁盘为固态硬盘 SSD，则建议设置为 `true`。
