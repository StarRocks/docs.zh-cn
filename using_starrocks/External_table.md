...
* 仅支持Iceberg的Catalog类型为Hive，数据存储格式为Parquet和ORC。
...
您需要提前创建Iceberg资源，该资源管理Iceberg数据源的相关连接信息。创建资源后，即可使用该资源创建外部表。
...
> 删除Iceberg资源会导致其对应的所有Iceberg外表不可用，但Apache Iceberg中的数据并不会丢失。如果您仍需要通过StarRocks查询Iceberg的数据，请重新创建Iceberg资源，Iceberg数据库和外表。
