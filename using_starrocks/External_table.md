...
* `driver_class`：JDBC 驱动程序的类名称，用于指定目标数据库使用的 JDBC 驱动程序。以下列举常见 JDBC 驱动程序的类名称：
...
执行如下命令，查看 StarRocks 中的所有 JDBC 资源：
...
执行如下命令，在 StarRocks 中创建并进入名为 `jdbc_test` 的数据库：
...
执行如下命令，在数据库 `jdbc_test` 中，创建一张名为 `jdbc_tbl` 的 JDBC 外部表：
...
* `resource`：JDBC 资源名称，必填项，必须和创建资源时指定的 JDBC 资源名称保持一致。
* `table`：目标数据库的表名，必填项。
