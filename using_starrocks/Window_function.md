...
窗口函数是 StarRocks 内置的特殊函数。和聚合函数类似，窗口函数通过对多个输入行进行计算得到一个数据值。不同的是，窗口函数是在一个特定的窗口内对输入数据做处理，而不是按照 Group By 来分组计算。每个窗口内的数据可以用 Over() 从句进行排序和分组。窗口函数会**对结果集的每一行**计算出一个单独的值，而不是对每个 Group By 分组计算一个值。这种灵活的方式允许您在 SELECT 从句中增加额外的列，给您提供了更多的机会来对结果集进行重新组织和过滤。
...
* **partition_by_clause**：Partition By 从句。与 Group By 从句类似，Partition By 从句将输入行按照指定的一列或多列分组，相同值的行会被分到一组。
...
`DENSE_RANK()` 函数用来为特定窗口中的数据排名。与 `RANK()` 不同的是，`DENSE_RANK()` 的返回中**不会出现空缺数字**。例如，在为数据 `1, 1, 2` 排名时，`DENSE_RANK()` 的返回中第三个数仍然是 `2`，而 `RANK()` 的返回中第三个数是 `3`。
...
`RANK()` 函数用来对当前窗口内的数据进行排名。与 `DENSE_RANK()` 不同的是，`RANK()` 的返回中**会出现空缺数字**。例如，在为数据 `1, 1, 2` 排名时，`RANK()` 的返回中第三个数是 `3`，而 `DENSE_RANK()` 的返回中第三个数是 `2`。
