...
| COMMENT          | 表注释。                                                     |
| partition_desc   | 分区方式。更多说明，请参见 [StarRocks 数据分布](../table_design/Data_distribution.md)。如果不填写，则默认为无分区。 |
| distribution_des | 分桶方式。更多说明，请参见 [StarRocks 数据分布](../table_design/Data_distribution.md)。如果不填写，则默认分桶键为CBO统计信息中最高基数的列，分桶数量为10。如果CBO中没有相关统计信息，则默认分桶键为第一列。 |
